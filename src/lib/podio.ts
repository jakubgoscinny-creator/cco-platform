 ...
 * Uses OAuth2 app authentication (client credentials flow).
 * Rate limit: 250 requests/hour for app auth.
 */
...
export async function filterItems(
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Podio filterItems app ${appId} failed (${res.status}): ${text}`);
  }
...
export async function updateItem(
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Podio updateItem ${itemId} failed (${res.status}): ${text}`);
  }
}
...
export async function createItem(
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Podio createItem app ${appId} failed (${res.status}): ${text}`);
  }
...
export function getFieldValue(
  ...
  const field = item.fields?.find(
    (f) =>
      f.field_id === Number(fieldId) ||
      f.external_id === String(fieldId)
  );
  if (!field || !field.values?.length) return undefined;
  return field.values;
}

export function getTextValue(item: PodioItem, fieldId: number | string): string {
  const vals = getFieldValue(item, fieldId);
  if (!Array.isArray(vals) || !vals.length) return "";
  return vals[0]?.value ?? "";
}

export function getCategoryValue(item: PodioItem, fieldId: number | string): string {
  const vals = getFieldValue(item, fieldId);
  if (!Array.isArray(vals) || !vals.length) return "";
  return vals[0]?.value?.text ?? "";
}

export function getCategoryValues(item: PodioItem, fieldId: number | string): string[] {
  const vals = getFieldValue(item, fieldId);
  if (!Array.isArray(vals)) return [];
  return vals.map((v) => v?.value?.text ?? "").filter(Boolean);
}

export function getNumberValue(item: PodioItem, fieldId: number | string): number | null {
  const vals = getFieldValue(item, fieldId);
...
export function getAppReferenceIds(item: PodioItem, fieldId: number | string): number[] {
  const vals = getFieldValue(item, fieldId);
  if (!Array.isArray(vals)) return [];
  return vals.map((v) => v?.value?.item_id).filter((id): id is number => id != null);
}
...
// Key field IDs for the Tests app
export const TEST_FIELDS = {
  TEST_NAME: 125981694,
  TEST_TYPE: 125981849,         // Static | Random
  ...
  TYPE: 137578152,              // CEU Quiz | Domain Pool | etc.
  TEST_RESULT_PROCESSING: 150300363,
  NEW_TEST_PLATFORM_STATUS: 273131496,
} as const;

// Key field IDs for QB Multi Choice
export const QUESTION_FIELDS = {
  TITLE: 126153194,
  QUESTION_TEXT: "question-text", // external_id varies, use field search
  DISPOSITION: 273026086,
  MANAGEMENT_STATUS: 126284767,
  PRACTICE_EXAM_DOMAINS: 16336321,
  TESTS: 137526907,
} as const;

// Key field IDs for Domains
...
// Key field IDs for Platform Profiles (auth)
